...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
  ...
  read -r -n1 -r -p "Press any key to continue..."
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
  ...
  # CRL is read -r with each client connection, while OpenVPN is dropped to nobody
...
server 10.8.0.0 255.255.255.0" > /etc/openvpn/server/server.conf
  ...
  case "$dns" in
    1|"")
      ...
      grep -v '^#\|^;' "$resolv_conf" | grep '^nameserver' | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}' | while read -r line; do
  ...
  case "$option" in
    ...
    2)
      ...
      if [[ "$revoke" =~ ^[yY]$ ]]; then
        ...
        # CRL is read -r with each client connection, when OpenVPN is dropped to nobody
        chown nobody:"$group_name" /etc/openvpn/server/crl.pem
        echo
        echo "$client revoked!"
      else
